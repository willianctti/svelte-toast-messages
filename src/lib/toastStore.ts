...
function validateOptions(options?: ToastOptions): ToastOptions {
    if (!options) return {};
    return {
        position: VALID_POSITIONS.includes(options.position as any) ? options.position : 'top-right',
        duration: options.duration ? Math.min(Math.max(options.duration, MIN_DURATION), MAX_DURATION) : DEFAULT_DURATION
    };
}
